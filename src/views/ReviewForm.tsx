...
const ReviewForm: FC<Props> = () => {
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async(evt) => {
    evt.preventDefault();
    if(!selectedRatings.length) return toast.error("Please select some rating!")
        try {
    setLoading(true)
            await client.post('/review', {bookId, rating: selectedRatings.length, content})
  ...
  return (
    <form onSubmit={handleSubmit} className="p-5 space-y-6">
      ...
      <RichEditor
        ...
        editable
      />
      <Button isLoading={loading} type="submit">Submit Review</Button>
    </form>
  );
};
