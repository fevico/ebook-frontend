...
return <div className='space-y-6 px-5 py-10'>
    {books.map((book) =>{
        return <div className='flex' key={book.id}>
            <img src={book.cover} alt={book.title} className='w-36 rounded' />
            <div className='p-5 space-y-3'>
                <h1 className='line-clamp-1 text-xl font-semibold'>{book.title}</h1>
                <div className="flex items-center space-x-1">
                    <span className='font-semibold'>By </span>
                <Link className='font-semibold hover:underline' to={`/author/${book.author.id}`}>
                 {book.author.name}
                </Link>
                </div>
                <div>
