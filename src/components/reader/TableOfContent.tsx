     ...
     className="dark:bg-book-dark dark:text-book-dark transition-all w-96 bg-white h-screen overflow-y-scroll fixed z-50 top-0 right-0 flex flex-col space-y-3 p-3 shadow-md">
      {data.map(({ label, subItems }) => {
        if (!subItems.length)
          return (
            <div key={label.title}>
              <p onClick={() => onClick(label.href)} className="py-2 text-large hover:underline cursor-pointer">
                {label.title}
              </p>
            </div>
          );
          ...
          <Accordion key={label.title} title={label.title}>
            <AccordionItem title={label.title}>
              <div className="space-y-3">
                {subItems.map((item) => {
                  return(
                  <p onClick={() => onClick(label.href)} key={item.title} className="pl-6 text-large hover:underline cursor-pointer">
                    {item.title}
                  </p>
                  )
                })}
