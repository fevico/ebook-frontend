...
const NotesModal: FC<Props> = ({ book, notes, isOpen, onClose, onNoteClick }) => {
  ...
  return (
    <Modal size="full" isOpen={isOpen} onClose={onClose}>
      <ModalContent>
        <ModalHeader className="flex items-center justify-center">Notes & Highlights</ModalHeader>
        <ModalBody className="flex flex-col gap-6 items-center">
            {data.map(item=>{
                return <Card className="max-w-2xl" key={item.cfi}>
                    <CardBody className="cursor-pointer" onClick={() => {
